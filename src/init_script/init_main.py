...
class InitialContainerCheck:
    def __init__(self, config_path: str, s3OTC: dict) -> None:
        ...
        self.s3 = s3fs.S3FileSystem(
            endpoint_url=self.s3OTC["FSSPEC_S3_ENDPOINT_URL"],
    ...
    def create_new_config(self):
        ...
        logger.debug("checking dataset availability")
        poped_ds = []
        ...
        for dataset, data in self.resources.items():
            ...
            if self.config["resources"][dataset]["providers"][0]["name"] in [
                "xarray-edr",
                "S3GeoJSONProvider.S3GeoJSONProvider",
            ]:
...
if __name__ == "__main__":
    config = os.environ.get(
        "PYGEOAPI_CONFIG",
        default="/pygeoapi/config/local.config.yml",
    )
    ...
    s3 = {
        "FSSPEC_S3_ENDPOINT_URL": os.environ.get(key="FSSPEC_S3_ENDPOINT_URL"),
