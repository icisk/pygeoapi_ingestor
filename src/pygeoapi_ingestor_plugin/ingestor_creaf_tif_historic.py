...
def tifs_to_ds(path, variable):
    ...
    time = sorted(
        set([np.datetime64(f"{parts[0]}-{parts[1]}-01") for fn in file_names for parts in [fn.split("_")[1:3]]])
    )
    LOGGER.debug("getting centereeeeeeeeeeeoids")
    x, y = get_pixel_centroids(files[0])
    # xarray creation
    da_list = []
    LOGGER.debug("reading tiffs")
    arrays = [tiff.imread(file) for file in files]
    LOGGER.debug("stackong arrays")
    stacked = np.stack(arrays, axis=0)
    da = xr.DataArray(stacked, dims=["time", "latitude", "longitude"], name=f"historic_{variable}")
    da_list.append(da)

    ds = xr.Dataset(
        {f"historic_{variable}": (["time", "latitude", "longitude"], da_list[i].data) for i in range(len(da_list))},
        coords={"time": time, "latitude": y, "longitude": x},
        attrs={"long_name": variable, "metrics": [t.astype(str) for t in time], "units": "mm"},
    )
...
class IngestorCREAFHISTORICProcessProcessor(BaseProcessor):
    ...
    def execute(self, data):
        ...
        outputs = {"id": "creaf_historic_ingestor", "value": self.zarr_out}
