...
from .utils import download_ftp_data, get_data_from_cloud, get_latest_forecast
...
PROCESS_METADATA = {
    ...
    "outputs": {
        ...
        "status": {"title": "status", "description": "shows if new data got downloaded", "schema": {"type": "string"}},
    },
    "example": {"inputs": {"zarr_out": "s3://example/target/bucket.zarr", "token": "ABC123XYZ666"}},
}
...
class RIJNLAND_evapo_dataprep_SMHI(BaseProcessor):
    ...
    def download_and_process(self, latest_fc_path):
        download_ftp_data(self.smhi_server, self.smhi_user, self.smhi_passwd, latest_fc_path, self.fc_save_path)
        ...
        tas_files = [
            os.path.join(fc_data_path, p)
            for p in sorted(os.listdir(fc_data_path))
            if p.startswith("tas") and p.endswith(".nc")
        ]
        pr_files = [
            os.path.join(fc_data_path, p)
            for p in sorted(os.listdir(fc_data_path))
            if p.startswith("pr") and p.endswith(".nc")
        ]
        ...
        res = [
            self.calc_delta_PET(forecast_xr.isel(time=i).tasAdjust, forecast_xr.isel(time=i).prAdjust, self.get_date(t))
            for i, t in enumerate(forecast_xr["time"].values)
        ]
    ...
    def execute(self, data):
        ...
        if self.fc_zarr_out and self.fc_zarr_out.startswith("s3://"):
            ...
            if s3.exists(self.fc_zarr_out):
                self.online_fc_data = get_data_from_cloud(
                    self.fc_zarr_out, self.alternate_root, self.otc_endpoint, self.otc_key, self.otc_secret
                )
            else:
                self.online_fc_data = None

        latest_fc_date, latest_fc_path = get_latest_forecast(
            self.smhi_server, self.smhi_user, self.smhi_passwd, self.smhi_fc_root
        )
        if self.online_fc_data:
            LOGGER.debug("forecast in otc")
            online_fc_date = datetime.datetime.fromtimestamp(
                self.online_fc_data["time"].data[0].astype("datetime64[s]").astype(int)
            )
            if online_fc_date < latest_fc_date:
                LOGGER.debug("new fc available")
                download_fc = True
            else:
                LOGGER.debug("no new fc available")
                # TODO: set back to false !!!!!!!!
                download_fc = True
        else:
            LOGGER.debug("no forecast in otc")
            download_fc = True

        if download_fc:
            LOGGER.debug("download from ftp")
            ...
            LOGGER.debug("upload to otc")
            s3 = s3fs.S3FileSystem()
            store = s3fs.S3Map(root=self.fc_zarr_out, s3=s3, check=False)
            forecast_xr.to_zarr(store=store, consolidated=True, mode="w")

            status_msg = "downloaded new forecast data"
        else:
            status_msg = "no new forecast available"

        outputs = {"id": "ll7_smhi_forecast", "value": self.fc_zarr_out, "status": status_msg}
