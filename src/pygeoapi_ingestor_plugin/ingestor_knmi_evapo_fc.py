...
class IngestorKNMIobsforecastProcessProcessor(BaseProcessor):
    ...
    def update_db_config(self):
        ...
        with lock:
            ...
            config["resources"][f"{self.title}"] = {
                "type": "collection",
                "title": f"{self.title}",
                "description": "pot precip deficit obs",
                "keywords": ["country"],
                "extents": {
                    "spatial": {
                        "bbox": [min_x, min_y, max_x, max_y],
    ...
    def homogenize_ds(self):
        kkk = xr.open_zarr(self.obs).rio.write_crs(28992)
        sss = xr.open_zarr(self.forecast).rio.write_crs(4326)
    ...
    def calc_p_def(self, pet0, pet1):
        p_def = np.where(
            np.logical_or(np.isnan(pet0), np.isnan(pet1)), np.nan, np.where((pet0 + pet1 > 0), pet0 + pet1, 0)
        )
        return p_def
    ...
    def calc_p_def_from_obs(self):
        ...
        final_xr = xr.DataArray(
            ...
            coords={
                ...
                "time": self.smhi_homo["time"].isel(
                    time=slice(len(self.smhi_homo["time"]) - result_array.shape[1], None)
                ),
    ...
    def execute(self, data):
        ...
        LOGGER.debug("checking online resources")
        ...
        LOGGER.info("homogenizing ds")
        self.knmi_homo, self.smhi_homo = self.homogenize_ds()
        LOGGER.info("calculating precip deficit")
        ...
        final_xr = xr.DataArray(
            p_def_10,
            dims=["time", "lat", "lon"],
            coords={"time": self.obs_forcast["time"], "lat": self.obs_forcast["lat"], "lon": self.obs_forcast["lon"]},
            name="p_def_q10",
        )
        ...
        LOGGER.info("creating contours")
        self.gdf = self.create_contours_df(self.obs_forcast)
        nl_landmasses_clip = gpd.read_file(
            "/pygeoapi/secondary_process_data/NL_landmasses_4326_50thdegreebuffer.geojson"
        )
        forecast_gdf = self.gdf.clip(nl_landmasses_clip)
        LOGGER.info("put vector data in db")
        ...
        outputs = {"id": "knmi_evapo_ingestor_obs_forecast", "value": self.fc_zarr_out}
