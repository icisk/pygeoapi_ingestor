...
class IngestorKNMIProcessProcessor(BaseProcessor):
    ...
    def download_date_data(
        self,
        date,
        header,
        params=dict(maxKeys=10, sorting="desc"),  # noqa: B006
        data_source=dict(  # noqa: B006
            precip={  # noqa: B006
    ...
    def execute(self, data):
        ...
        if self.mode == "append":
            date = datetime.datetime.fromtimestamp(
                self.online_obs_data.time.values[-1].astype("datetime64[s]").astype(int)
            )
        ...
        res = xr.Dataset(
            data_vars=dict(p_def=(["time", "y", "x"], p_def)), coords=dict(time=t.time.data, y=t.y.data, x=t.x.data)
        )
        ...
        if self.mode == "init":
            ...
            new_res = xr.Dataset(
                ...
                coords=dict(time=t.time.data, y=t.y.data, x=t.x.data),
            )
        ...
        try:
            ...
            raise ProcessorExecuteError(f"""could not create tif from nc '{e}'""") from e

        # FIXME the following variables are not accessed, hence commented
        # if self.mode == 'init':
        ...
        try:
            ...
            LOGGER.error(f"Error: {e}")
            raise ProcessorExecuteError(f"""could not create contours from geojson '{e}'""") from e
        ...
        outputs = {"id": "knmi_evapo_ingestor", "value": self.obs_zarr_out}
