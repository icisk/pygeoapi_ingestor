...
_living_lab_basin = {"georgia": "data/georgia_basins.geojson"}
...
def validate_parameters(data, data_type):
    ...
    if cron_invocation:
        if data_type == "historic":
            period_of_interest = (datetime.datetime.now() - relativedelta.relativedelta(months=1)).strftime(
                "%Y-%m"
            )  # INFO: previous month .. historic data for current month will never exists
        ...
        try:
            ...
            if period_of_interest.strftime("%Y-%m") >= datetime.datetime.now().strftime("%Y-%m"):
                raise Handle200Exception(
                    Handle200Exception.SKIPPED, "period_of_interest must be a date before current date month"
                )
        except ValueError as e:
            raise ProcessorExecuteError("period_of_interest must be a valid datetime YYYY-MM string") from e
        period_of_interest = [
    ...
    def validate_period_of_interest_forecast(period_of_interest):
        ...
        try:
            ...
            if period_of_interest.strftime("%Y-%m") == datetime.datetime.now().strftime(
                "%Y-%m"
            ...
            if period_of_interest.strftime("%Y-%m") > datetime.datetime.now().strftime("%Y-%m"):
                raise Handle200Exception(
                    ...
                    f"period_of_interest of {period_of_interest.strftime('%Y-%m')} will be available from day {period_of_interest.strftime('%Y-%m')}-06 at 12UTC",
                )

            if diff_months(datetime.datetime.now(), period_of_interest) > 6:
                raise Handle200Exception(
                    Handle200Exception.SKIPPED, "period_of_interest must be within 6 months from current date"
                )
        except ValueError as e:
            raise ProcessorExecuteError("period_of_interest must be a valid datetime YYYY-MM string") from e
        period_of_interest = [
            period_of_interest.date(),
            (period_of_interest + datetime.timedelta(hours=5160))
            .replace(day=1, hour=0)
            .date(),  # INFO: If we want all the forecast data of the declared month: 6 month # REF: https://cds.climate.copernicus.eu/datasets/seasonal-original-single-levels?tab=download#leadtime_hour
    ...
    def check_s3_path_exists(living_lab, period_of_interest, data_type):
        ...
        pygeoapi_zarr_collection_id = _collection_pygeoapi_identifiers[living_lab][data_type](
            date=period_of_interest[0]
        )
        if s3_zarr_collection_uri.startswith("s3://"):
            ...
            if s3.exists(s3_zarr_collection_uri):
                if pygeoapi_zarr_collection_id in utils.read_config(_config_file)["resources"]:
                    raise_200_exception = {
                        "status": Handle200Exception.OK,
                        "message": f"Path {s3_zarr_collection_uri} already exists in bucket and config",
                    }
                else:
                    ...
                    collection_params = {
                        ...
                        "bbox": [
                            ds.lon.values.min().item(),
                            ds.lat.values.min().item(),
                            ds.lon.values.max().item(),
                            ds.lat.values.max().item(),
                        ],
                        "time": {"begin": ds.time.min().dt.date.item(), "end": ds.time.max().dt.date.item()},
                    ...
                    raise_200_exception = {
                        ...
                        "message": f"Path {s3_zarr_collection_uri} already exists in bucket, updates config at '{_config_file}'",
                    }

        if data_type == "forecast" and _living_lab_basin.get(living_lab, None) is not None:
            s3_zonal_stats_collection_uri = _s3_spi_collection_zonal_stats_uris[living_lab][data_type](
                date=period_of_interest[0]
            )
            ...
            if s3_zonal_stats_collection_uri.startswith("s3://"):
                ...
                if s3.exists(s3_zonal_stats_collection_uri):
                    if pygeoapi_zonal_stats_collection_id in utils.read_config(_config_file)["resources"]:
                        ...
                        raise_200_exception = {
                            "status": raise_200_exception["status"]
                            if raise_200_exception.get("status", None)
                            else Handle200Exception.OK,
                            "message": f"{raise_200_exception['message']}. {raise_msg}"
                            if raise_200_exception.get("message", None)
                            else raise_msg,
                        }
                    else:
                        if s3.exists(s3_zarr_collection_uri):
                            ...
                            geojson_collection_params["pygeoapi_id"] = (
                                f"{zarr_collection_params['pygeoapi_id']}_zonal_stats"
                            )
                    ...
                    if s3.exists(s3_zarr_collection_uri):
                        ...
                        geojson_collection_params = create_s3_zonal_stats_collection_data(
                            living_lab, ds_zonal_stats, zarr_collection_params, data_type="forecast"
                        )
                        update_config(living_lab, geojson_collection_params)
...
def grib2xr(grib_filename, grib_var_name, xr_var_name=None):
    ...
    var_data = []
    times_range = []
    for _, msg in enumerate(grib_ds_msgs):
        values, _, _ = msg.data()
    ...
    xr_dataset = xr.Dataset(
        ...
        coords={"time": times_range, "lat": lat_range, "lon": lon_range},
    )
    return xr_dataset
...
def compute_timeseries_spi(monthly_data, spi_ts, nt_return=1):
    ...
    G = lambda x: stats.gamma.cdf(x, a=a, loc=0, scale=b)  # noqa: E731

    m = (t_scaled_monthly_data == 0).sum()
    n = len(t_scaled_monthly_data)
    q = m / n  # zero prob

    H = lambda x: q + (1 - q) * G(x)  # zero correction  # noqa: E731

    t = lambda Hx: math.sqrt(  # noqa: E731
        math.log(1 / (math.pow(Hx, 2) if 0 < Hx <= 0.5 else math.pow(1 - Hx, 2)))
    )
    ...
    Z = lambda Hx, tx: (  # noqa: E731
        tx - ((c0 + c1 * tx + c2 * math.pow(tx, 2)) / (1 + d1 * tx + d2 * math.pow(tx, 2) + d3 * math.pow(tx, 3)))
    ) * (-1 if 0 < Hx <= 0.5 else 1)  # noqa: E731
...
def update_config(living_lab, collection_params):
    ...
    with lock:
        ...
        dataset_definition = {
            ...
            "keywords": ["country"],
            "extents": {
                "spatial": {"bbox": collection_params["bbox"], "crs": "http://www.opengis.net/def/crs/OGC/1.3/CRS84"},
                "temporal": {
...
def compute_zonal_stats(living_lab, spi_dataset):
    ...
    def basin_zonal_stat(ds, basin_gdf, stat_name, stat_func, stat_func_kwargs=None):
        if stat_func_kwargs is None:
            stat_func_kwargs = {}
...
def create_s3_zonal_stats_collection_data(living_lab, zonal_stat_gdf, spi_coverage_collection_params, data_type):
    ...
    s3_zonal_stats_collection_uri = _s3_spi_collection_zonal_stats_uris[living_lab][data_type](
        date=spi_coverage_collection_params["time"]["begin"]
    )
...
def coverage_to_out_format(coverage_ds, out_format):
    ...
    if out_format == "zarr":
        ...
        shutil.make_archive(
            coverage_zarr_zip_filepath, "zip", coverage_zarr_filepath
        )  # Compress to .zip to transmit as bytes
