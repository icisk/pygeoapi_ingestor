...
PROCESS_METADATA = {
    ...
    'inputs': {
        'data_url': {
            'title': 'data url',
            'description': 'The URL of the gpkg',
            'schema': {
                'type': 'string'
            }
        },
        'll': {
            'title': 'living lab name',
            'description': 'll prefix',
            'schema': {
                'type': 'string'
            }
        },
    ...
    'example': {
        "inputs": {
            "data_url": "s3://example/target/ll.gpkg",
            "variable": "ll_spain",
            "token": "ABC123XYZ666"
        }
    }
...
class IngestorLLGPKGProcessProcessor(BaseProcessor):
    ...
    def execute(self, data):
        ...
        for l in layers['name']:
            print(l)
            layer_dict[l] = gpd.read_file(self.data_path, layer=l)
